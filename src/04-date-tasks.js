...
function timeSpanToString(startDate, endDate) {
  return new Date(endDate - startDate).toJSON().substring(11, 23);
}
...
function angleBetweenClockHands(date) {
  ...
  hours *= 30;
  hours += date.getUTCMinutes() / 2;
  return (Math.PI / 180) * Math.min(Math.abs(hours - minutes), 360 - Math.abs(minutes - hours));
}
